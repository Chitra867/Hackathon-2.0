...
def broadcast_availability_update(hospital_id, availability_data):
    """Broadcast an availability update to hospital and global channels."""
    try:
...
class HospitalViewSet(viewsets.ModelViewSet):
    ...
    Public:
    - List and retrieve hospitals, subject to queryset filtering.

    Hospital administrators:
    - PATCH contact and address fields for their own hospital.

    System administrators:
    - Create, update, and deactivate hospitals.
    """
    ...
    def get_queryset(self):
        ...
        if (
            ...
            or user.role not in ('hospital_admin', 'system_admin')
        ):
            queryset = queryset.filter(
                is_active=True,
    ...
    def get_serializer_class(self):
        ...
        if (
            ...
            and getattr(self.request.user, 'role', None) == 'hospital_admin'
        ):
...
class ServiceViewSet(viewsets.ModelViewSet):
    ...
    Write operations require a system administrator.
    """
...
class HospitalServiceViewSet(viewsets.ModelViewSet):
    """
    Manage links between hospitals and services.
...
class AvailabilityViewSet(viewsets.ModelViewSet):
    ...
    GET /api/availability/ is public.
    Hospital staff/admins can create and update their availability.
    Bulk updates use /api/availability/bulk-update/.
    """
    ...
    def perform_create(self, serializer):
        ...
        AuditLog.log(
            ...
            description=(
                ...
                f"set to '{availability.get_status_display()}'."
            ),
            ip_address=get_client_ip(self.request),
        )
    ...
    def perform_update(self, serializer):
        ...
        AuditLog.log(
            ...
            description=(
                f"Availability for '{availability.hospital.name}' "
                f"({availability.get_availability_type_display()}) "
                f"updated to '{availability.get_status_display()}'."
            ),
            ip_address=get_client_ip(self.request),
        )
        ...
        with transaction.atomic():
            for update_data in updates:
                ...
                if request.user.role in (
                    ...
                    if (
                        request.user.hospital is None
                        or request.user.hospital_id != hospital.id
                    ):
                        return Response(
                            {
                                'error': (
                ...
                results.append(
                    AvailabilitySerializer(availability_obj).data,
                )
